...
# ejecutar como ejemplo "mueve_archivos_antiguos_bads.sh /home1/sigma/bads/ 90"
...
cd $1
find . -name "*.err" -type f -mtime +$2 -exec ls {} \; > salida.txt
cant=`cat salida.txt | wc -l`
cont=1
while [ $cont -le $cant ] ; do
      arch=`head -$cont salida.txt | tail -1`
      mv $arch /home1/sigma/bads/tmp_wwrl
      compress -v /home1/sigma/bads/tmp_wwrl/$arch
      cont=`expr $cont + 1`;
done
#cd $1
#cd tmp
