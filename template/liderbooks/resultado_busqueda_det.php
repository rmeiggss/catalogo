...
for($k=0;$k<count($campos);$k++)
	{
		...
		{
			$varcabecera.="<td style='color:#333333; font:Verdana, Arial, Helvetica, sans-serif; font-size:13px;' class='celdatitulo' width='".$campos[$k][2]."%'><strong><a title='Ordenar (en orden de menor a mayor) por el campo ".$elemento."' class='celdatitulolink' href='javascript:ordena(".chr(34).$campos[$k][0].chr(34).")' >".strtoupper($elemento)."</a></strong> <a title='Ordenar (en orden de mayor a menor) por el campo ".$elemento."' class='celdatitulolink' href='javascript:ordena(".chr(34).$campos[$k][0]." desc ".chr(34).")'>^</a></td>".chr(13).chr(10);
		}
		else
		{
			$varcabecera.="<td style='font:bold' class='celdatitulo' width='".$campos[$k][2]."%'>&nbsp;</td>".chr(13).chr(10);
		};
		$total+=$campos[$k][1];
	}
$varcabecera.="<td style='color:#333333; font:Verdana, Arial, Helvetica, sans-serif; font-size:13px' class='celdatitulo' width='".(80-$total)."%' align='center'><strong>ACCIONES</strong></td>";
$vardetalle="";
...
while ($row = mysql_fetch_array($rs))
	{
		...
		$fila="<tr onclick='probar(".chr(34).$row[0].chr(34).")'>";
		...
		for ($k=0;$k<count($campos);$k++)
		{
			...
			$fila.="    <td align='left' style='color:#333333; font:Verdana, Arial, Helvetica, sans-serif; font-size:11px;' ".$alinea.$anadir." class='celdadetalle' >".(strlen($alinea)==0 ? $row[$k] :  number_format($row[$k],2) )."&nbsp;</td>";
		};
...
<form name="a" method="post">
		<table cellpadding='2' cellspacing='8' border="0" width="100%">
			<tr>
				<td style="font:Verdana, Arial, Helvetica, sans-serif; font:bold; font-size:14px; color:#990000" class="titulotabla" >
					<?=$display;?>
				</td>
			</tr>
		</table>
		<p align='center'>
			<table style='display:none;' class="tablabotones" border="0" width="95%" cellspacing="0" cellpadding="6">
				<tr>
...
for (i=0;i<l;i++)
	{
		r3[i].onmouseover = function(){this.style.backgroundColor= (document.body.bgColor==""  ? "#FFCC00" : document.body.bgColor ) };
...
function imprime()
	...
	//alert(variable);
	location.href="editarproducto.php?libro="+variable;
	}
