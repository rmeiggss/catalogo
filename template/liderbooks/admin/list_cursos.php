...
//Combo categoria
$sql_cat="select categoria,nombre from categoria where borrado=''";
$rs_cat=mysql_query($sql_cat);
while($row_cat=mysql_fetch_array($rs_cat)){
    $id=$row_cat[0];
    $arrCombo[$id]=$row_cat[1];
}
$o_Combo = new Combo($arrCombo);
$comboHTML = $o_Combo->getOptionsHTML('000001');
$listaCategoria=$o_Combo->getSelecccionHTML($comboHTML,'cbCategoria','','onchange','');
//
$sql_news="select curso,nombre,categoria,descripcion from curso where borrado='0'";
...
while($row_news=mysql_fetch_array($rs_news)){
    if($j>=$inicio & $j<=$fin){
        $id=$row_news['curso'];
        $fecha=$row_news['nombre'];
        $titulo=$row_news['categoria'];
        $contenido=$row_news['descripcion'];
        $fila.="<tr class='m1'>";
        $fila.="<td align='center'>".$j."</td>";
        $fila.="<td align='left'>".$fecha."</td>";
        $fila.="<td align='center'>".$titulo."</td>";
        $fila.="<td align='center'><input type='button' name='btn' onclick='javascript:editar($id);' value='Editar'>&nbsp;<input type='button' name='btn' onclick='javascript:eliminar($id);' value='Eliminar'></td>";
        $fila.="</tr>";
    }
    ++$j;
}
...
<tr>
    ...
    <td width="640" align="center" valign="top">
        ...
        <p align='left'>&nbsp;&nbsp;Categoria: <?php echo $listaCategoria;?></p>
        <table width="98%" cellpadding="0" cellspacing="0" border="1">
        <tr class="m6">
           <td width="20" align="center">No</td>
           <td width="300" align="center">Cursos</td>
           <td width="50" align="center">Fecha</td>
           <td width="120" align="center">Acciones</td>
        </tr>
        <?php echo $fila;?>
        </table>
       <div align="right">
